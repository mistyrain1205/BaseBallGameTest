...
class Player {
    constructor(id, forcedPos = null, fixedData = null) {
        ...
        // サブポジションの生成
        const allPos = ["捕", "一", "二", "三", "遊", "左", "中", "右"];
        const others = allPos.filter(p => p !== this.mainPos);
        const subCount = rand(3, seed + 50);
        this.subPos = [];
        for (let i = 0; i < subCount; i++) {
            const p = others[rand(others.length, seed + 60 + i)];
            if (!this.subPos.includes(p)) this.subPos.push(p);
        }
    }
    get rankClass() {
    ...
    getFourSeam() {
        ...
        const choices = Object.keys(this.pitches).filter(name => PITCH_DATA[name].category === 'Fastball');
        if (choices.length === 0) return 'フォーシーム';
        return choices.sort((a, b) => this.pitches[b] - this.pitches[a])[0];
    }

    // 変化球系の最高レベル球種を取得
    getBestBreakingBall() {
        if (!this.pitches) return null;
        const choices = Object.keys(this.pitches).filter(name => PITCH_DATA[name].category !== 'Fastball' && PITCH_DATA[name].category !== 'Junk');
        if (choices.length === 0) return Object.keys(this.pitches)[0];
        return choices.sort((a, b) => this.pitches[b] - this.pitches[a])[0];
    }
}
...
class GameState {
    ...
    executeOnePitch(batter, pitcher, pCmd, bCmd) {
        ...
        let pitchName;
        if (pCmd === 'Strike') {
            pitchName = pitcher.getFourSeam();
        } else if (pCmd === 'Kiwadoi') {
            pitchName = pitcher.getBestBreakingBall();
        } else {
            const pitchList = Object.keys(pitcher.pitches);
            pitchName = pitchList[rand(pitchList.length)];
        }
...
class BaseballSimulation {
    ...
    startGame() {
        ...
        // CPU側：スタメン以外の全選手をベンチへ
        this.state.cpuLineup = this.cpuTeam.players.filter(p => p.mainPos !== "投").slice(0, 9);
        this.state.cpuBench = this.cpuTeam.players.filter(p => p !== this.state.cpuPitcher && !this.state.cpuLineup.includes(p));

        // プレイヤー側：スタメン以外の全選手をベンチへ
        this.state.bench = this.myTeam.players.filter(p => p !== this.starter && !this.lineup.includes(p));
    ...
    toggleWarmUp(id) {
        ...
        if (p) {
            if (!p.isWarmingUp) {
                const activeCount = this.state.bench.filter(x => x.isWarmingUp).length;
                if (activeCount >= 2) {
                    alert("ブルペンで同時に練習できるのは2人までです。");
                    return;
                }
            }
            p.isWarmingUp = !p.isWarmingUp;
        }
        this.showBullpen();
    }
    ...
    showStats() {
        ...
        disp.innerHTML = `
            <h3>${p.name}</h3>
            <div class="role-tag">${p.playerType} / ${p.mainPos}${p.subPos.length > 0 ? ` (サブ: ${p.subPos.join('')})` : ''}</div>
