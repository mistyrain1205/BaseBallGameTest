...
class GameState {
    ...
    executeOnePitch(batter, pitcher, pCmd, bCmd) {
        ...
        if (!doesSwing) {
            ...
            let contactRate = (batter.stats.meet * 0.8) + (activeCtl * 0.1);
            if (pCmd === 'Strike') contactRate += 10;
            if (bCmd === 'Aggressive') contactRate -= 10;
            if (bCmd === 'Bunt') contactRate = 90;

            if ((Math.random() * 100) > contactRate) return 'SwingingStrike';
            if (Math.random() < 0.4) return 'InPlay';
            return 'Foul';
        }
    }

    processPitchResult(res, batter, pitcher, fielders) {
        ...
        switch (res) {
            ...
            case 'Foul':
                ...
                break;
            case 'InPlay':
                this.addLog(msg + (bCmd === 'Bunt' ? "バントした！" : "打った！"), "hit");
                this.resolveInPlay(batter, pitcher, fielders, bCmd === 'Bunt');
...
class BaseballSimulation {
    ...
    issueCommand(type, cmd) {
        ...
        this.state.processPitchResult(res, batter, pitcher, defenders);
        this.state.updateBullpen();
        this.updateBatterDisplay();
    }
